...
function saveRecording(blob, encoding) {
  ...
  console.log(blob);
  var fd = new FormData();
  fd.append('fileName', +new Date() + '.wav');
  fd.append('audioFile', blob);
  fd.append('type', $('#r_type').val());
  fd.append('id', $('#r_id').val());
  fd.append('instance_id', $('#r_instance_id').val());
  fd.append('message', $('#r_message').val());
  $.ajax('https://speakez-somali.syr.edu/freedomfone/ivr_menus/record', {
    method: 'POST',
